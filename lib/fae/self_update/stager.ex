defmodule Fae.SelfUpdate.Stager do
  @moduledoc """
  Validates and extracts a Fae release tarball into a staging directory
  — the contract-validation step between "bytes arrived" and "shell
  ...
  Every entry in the tarball is validated **before** extraction:
    ...
    * Symlinks and hard links are rejected (mix releases don't need
      them — accepting them would open directory escape via link-swap
      races).
    * Non-regular, non-directory entries (device files, FIFOs,
      sockets) are rejected.
    * Cumulative declared size is capped — any tarball claiming to
      expand beyond `:max_bytes` aborts extraction.

  After a successful extraction, the required mix-release members are
  checked for presence — if the bundled installer or systemd unit is
  missing, handoff is aborted rather than handing off to an installer
  that won't know how to migrate.

  Staging dirs are created with `0o700` so another local process can't
  read in-flight extracted artifacts.
  """
  ...
  Options:
    ...
    * `:required` — list of relative paths that must be present in the
      extracted tree. Default covers the bundled installer, the release
      runner, and the systemd unit.
  """
  ...
  @doc """
  Pure validator — exposed for unit testing against synthetic entry
  tuples so the test suite doesn't need to craft malformed tarballs.
  """
  ...
  # we're called, so we must not rm_rf it. Each staging dir has a
  # random suffix; no prior-run cleanup needed.
